...
class Fun(CogMeta):
    ...
    @example(",button Click Me! Hello! blurple 60")
...
class ActionButton(ui.View):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        async with cog.session.get(f"https://nekos.best/api/v2/{self.action}") as response:
...
class Roleplay(CogMeta):
    def __init__(self, bot):
        self.bot = bot
        self.session = aiohttp.ClientSession(headers={"User-Agent": "Xrypton (https://xrypton.vercel.app)"})
    ...
    async def roleplay(self, ctx):
        ...
        embed.add_field(
            name="Usage",
            value=f"Use `{ctx.prefix}kiss [@user]` or `/roleplay kiss [user]`",
            inline=False,
        )
