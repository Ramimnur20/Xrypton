...
class Server(CogMeta):
    ...
    @hybrid_group(
        name="autoreact",
        aliases=["areact", "autoreacter", "reacter"],
        invoke_without_command=True,
        description="Configure autoreactions",
    )
    @has_permissions(manage_messages=True)
    ...
    @autoreact.command(name="add", description="Add an autoreaction")
    @has_permissions(manage_messages=True)
    async def autoreact_add(self, ctx: Context, trigger: str, emoji: str):
        check = await self.bot.pool.fetchrow(
            "SELECT * FROM autoreacts WHERE guild_id = $1 AND trigger = $2",
            ctx.guild.id,
            trigger.lower(),
        )
        if check:
            return await ctx.warn(
                f"An autoreact with the trigger `{trigger}` already exists."
            )

        await self.bot.pool.execute(
            "INSERT INTO autoreacts (guild_id, trigger, emoji, exclusive_user_id) VALUES ($1, $2, $3, $4)",
            ctx.guild.id,
            trigger.lower(),
            emoji,
            None,
        )
        return await ctx.approve(f"Added autoreact: `{trigger}` → {emoji}")

    @autoreact.command(name="edit", description="Edit an autoreaction emoji")
    @has_permissions(manage_messages=True)
    async def autoreact_edit(self, ctx: Context, trigger: str, new_emoji: str):
        check = await self.bot.pool.fetchrow(
            "SELECT * FROM autoreacts WHERE guild_id = $1 AND trigger = $2",
            ctx.guild.id,
            trigger.lower(),
        ...
        if not check:
            return await ctx.warn(
                f"No autoreact found with the trigger `{trigger}`."
            )

        await self.bot.pool.execute(
            "UPDATE autoreacts SET emoji = $1 WHERE guild_id = $2 AND trigger = $3",
            new_emoji,
            ctx.guild.id,
            trigger.lower(),
        )
        return await ctx.approve(
            f"Updated autoreact `{trigger}` emoji to {new_emoji}"
        )

    @autoreact.command(name="remove", description="Remove an autoreaction")
    @has_permissions(manage_messages=True)
    async def autoreact_remove(self, ctx: Context, trigger: str):
        ...
        return await ctx.approve(f"Removed autoreact for trigger `{trigger}`")

    @autoreact.command(name="list", description="List all autoreactions in this guild")
    @has_permissions(manage_messages=True)
    async def autoreact_list(self, ctx: Context):
        rows = await self.bot.pool.fetch(
            "SELECT trigger, emoji, exclusive_user_id FROM autoreacts WHERE guild_id = $1",
            ctx.guild.id,
        )

        if not rows:
            return await ctx.warn("There are no autoreactions set up in this guild.")

        entries = []
        for i, row in enumerate(rows, start=1):
            exclusive = (
                f" (exclusive: <@{row['exclusive_user_id']}>)"
                if row["exclusive_user_id"]
                else ""
            )
            entries.append(
                f"`{i}` **{row['trigger']}** → {row['emoji']}{exclusive}"
            )

        embeds = []
        embed = discord.Embed(
            color=COLORS.neutral, title="Autoreactions", description=""
        )
    ...
    @autoreact.command(
        name="exclusive", description="Set an autoreact to only trigger for a specific user"
    )
    @has_permissions(manage_messages=True)
    async def autoreact_exclusive(self, ctx: Context, trigger: str, user: discord.Member):
        check = await self.bot.pool.fetchrow(
            "SELECT * FROM autoreacts WHERE guild_id = $1 AND trigger = $2",
            ctx.guild.id,
            trigger.lower(),
        ...
        await self.bot.pool.execute(
            "UPDATE autoreacts SET exclusive_user_id = $1 WHERE guild_id = $2 AND trigger = $3",
            user.id,
            ctx.guild.id,
            trigger.lower(),
        )
        return await ctx.approve(
            f"Set autoreact `{trigger}` to exclusive for {user.mention}"
        )

    @Cog.listener("on_message")
    async def dispatch_autoreact(self, message: discord.Message):
        if message.author.bot:
            return
        ...
        for row in rows:
            if row["trigger"].lower() in message.content.lower():
                if row["exclusive_user_id"] is not None:
                    if message.author.id != row["exclusive_user_id"]:
                        continue
                try:
    ...
    @example(",fakeperms")
    @hybrid_group(name="fakeperms", aliases=["fp", "fakepermission", "fakepermissions"], invoke_without_command=True)
