...
class Information(CogMeta):
    ...
    async def userinfo(
        ...
        if user is None:
            user = ctx.author
        if isinstance(user, (User, Member, ClientUser)):
    ...
    @command(
        name="servericon",
        aliases=["servicon", "servico", "sicon"],
        description="Get the server's icon in full detail.",
    )
    ...
    @command(
        name="serverbanner",
        aliases=["servbanner"],
        description="Get the server's banner if present.",
    )
    ...
    async def status(self, ctx: Context):
        return await ctx.send(
            f"{ctx.author.mention}: Experiencing issues? Check your shards status on https://xrypton.vercel.app/status"
        )
